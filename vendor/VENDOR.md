...
| `vendor/endo/tui/TerminalQuery_test.cpp` | New case: the reply bytes `CSI 6;20;10t` go through `VtParser` into one protocol report, and `queryCellSize()` answers 10 wide and 20 high from it in one read. Test only | #134: the Sixel rung is chosen only with a measured cell size, and the reply is height first while the query answers width first. Every earlier case scripted an already-decoded `CellSizeReport`, so a swap on either side passed all of them | endo (contour-terminal/endo) | `D:/endo` branch `fastcached/upstream`, commit `f774a210ce989e5947b8f61d715068b1dc96088c`, on `4f8f4630`; unpublished, pending owner |
