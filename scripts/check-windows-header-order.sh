...
# Prefer the explicitly versioned binary, exactly as `local-gate.sh` does, then fall
# back to a bare one whose version is CHECKED below rather than assumed.
Formatter=""
...
VersionText="$("$Formatter" --version 2>&1)"
FoundVersion="$(printf '%s\n' "$VersionText" | sed -n 's/.*clang-format version \([0-9][0-9]*\).*/\1/p' | sed -n '1p')"
if [ -z "$FoundVersion" ]; then
    echo "FAIL: could not read a version from '$Formatter --version': $VersionText" >&2
    exit 1
fi
if [ "$FoundVersion" != "$PinnedVersion" ]; then
    # A REFUSAL rather than a skip, deliberately. Successive releases disagree about
    # include ordering, so this would be a verdict about a tool nobody runs -- and it
    # would most likely be a PASSING one, which is the direction that gets believed.
    echo "FAIL: found $Formatter at major version $FoundVersion, but CI pins $PinnedVersion." >&2
    echo "      An older or newer clang-format disagrees about include order, so its verdict" >&2
    echo "      here would describe a tool nobody runs. Install clang-format-$PinnedVersion" >&2
    echo "      (pip download clang-format==$PinnedVersion.1.0) rather than skipping this." >&2
    exit 1
fi
