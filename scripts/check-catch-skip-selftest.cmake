...
function(fastcached_run_check tree outObjected outOutput)
    ...
    # The verdict is read from the OUTPUT rather than the exit code, and the reason
    # is no longer the one that used to be written here. `message(FATAL_ERROR)` exits
    # nonzero on 3.28.3 as on 4.x -- the claim that 3.28 exits 0 did not reproduce
    # (#565) -- so the status would in fact work. Output is still what is read,
    # because this selftest asserts the check said the RIGHT THING, and an exit code
    # cannot distinguish the intended refusal from a CMake syntax error in the check
    # itself.
