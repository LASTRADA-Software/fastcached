...
legs="$(python3 - "$repo/.github/workflows/build.yml" <<'LEGS'
import re, sys
# A leg is a job whose steps invoke the sweep. Scanned line by line, tracking the
# current job key, rather than split on a regex -- the first version split on the
# job separator and the split CONSUMED the indent the match then required, so it
# found nothing and the check refused a correct tree.
#
...
for line in open(sys.argv[1]):
    ...
    if m:
        job = m.group(1)
    elif job and "tidy-sweep.sh --ci" in line:
        if job not in found:
            found.append(job)
for j in found:
    print(j)
