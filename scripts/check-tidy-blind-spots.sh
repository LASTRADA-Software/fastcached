...
for j in found:
    print(j)
LEGS
)" || refuse "a step runs tidy-sweep.sh in a mode this check does not recognise; see UNKNOWN-SWEEP-MODE above. A mode it cannot classify would make a real leg invisible and refuse every row naming it."
