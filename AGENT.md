...
- A credential lives in `SecureByteBuffer`, and the wipe is an **allocator**, not a destructor.
  Container-agnostic is not SUFFICIENT -- SSO keeps a short secret where no allocator is
  called. So a secret is never a `std::basic_string`: `SecureString` holds its characters in
  a `std::vector`, which has no inline footprint to wipe and no INLINE-TO-HEAP transition to
  miss. Holders are found by NAME, so a row that has stopped matching is a refusal.
...
- `--slots=0` means no WORKER, and `OfferableSlots` DERIVES from an absent count, so the flag
  ...
  `scope` column -- never a pasted `RunsWorker(c) &&`. `--scheduler` is not worker-only. The
  worker is `WorkerTier`, null there; the merged listener's bind, ceiling fold and connection sum all
  read `FamilyRoutes`, whose presence column every family must state -- never a list of members -- or
  a consensus-only node closes every connection.
...
- **And `CLANG_TOOLS_VERSION` pins a MAJOR, not a BUILD.** apt.llvm.org ships rolling snapshots
  ...
  of that session, not just the one file.
...
- An instruction-set extension is used only inside a function that asks for it
  ...
  `ctest -R shipping-configure-guard`.
...
- `clang-format -i` at any build but the pinned one silently reformats code the pinned one already
  accepted; run another binary as `--dry-run` only. The pinned formatter IS a PyPI build and
  clang-tidy ships there too, so "the distro only has an older one" is not a reason to use it. An
  older clang-tidy is worse than a laxer one: it is *silent* about checks that do not exist in it yet.
