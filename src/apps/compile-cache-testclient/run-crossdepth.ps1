...
function Invoke-CrossDepth([string]$compiler, [string]$deepSrc, [string]$shallowSrc, [string]$key, [string]$prefetch group) {
    ...
    try {
        ...
        & $Client store --port $Port --key $key --prefetch-group $prefetch group `
