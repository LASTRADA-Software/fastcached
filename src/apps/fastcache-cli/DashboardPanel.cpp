...
namespace
{
    ...
    /// How a figure is dressed: an alert when it is past its threshold, @p otherwise when it is not.
    /// @param figure The figure.
    /// @param value Its newest value.
    ...
    {
        return Alarming(figure, value) ? std::optional<FrameTone> { FrameTone::Alert } : otherwise;
    }
