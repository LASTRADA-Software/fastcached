...
TEST_CASE("A note's prefix is read back out of the probe's own output")
{
    ...
    CHECK(*learned == FastCache::PathCanon::IncludeNoteMarker);
}
...
TEST_CASE("A localized note's prefix is read back, which is the whole point")
{
    ...
    auto const learned = MarkerFromProbeOutput(output, ProbeHeader);
    REQUIRE(learned.has_value());
    CHECK(*learned == "Hinweis: Einlesen der Datei:");
}
...
TEST_CASE("A probe that would not say falls through to the English default")
{
    ...
    CHECK(ResolveIncludeNoteMarker("", &empty).source == MarkerSource::Default);
}
