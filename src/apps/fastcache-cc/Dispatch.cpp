                        ...
                        Wire::CodecList const& acceptedCodecs)
{
    ...
    auto worker = dialer.Dial(endpoint);
    if (worker == nullptr)
        // The lease is left to expire rather than being returned. There is no verb
        // for handing one back, and adding one would buy little: the scheduler
        // reclaims it on its own, and a client that cannot reach the worker is
        // exactly the client least able to report anything about it.
        return Refused(DispatchStatus::Unavailable, std::format("worker {} unreachable", endpoint));

    auto const argsField = EncodeArgs(request.args);
    ...
    if (!compileOutcome.IsHit())
        ...
        return Refused(DispatchStatus::Declined,
                       std::format("{} refused the job: {}", endpoint, DescribeOutcome(compileOutcome)));
