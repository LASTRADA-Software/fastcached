...
inline constexpr std::string_view ConsensusNeedsClusterKeyRefusal =
    ...
    "the leader's does, but it still proves a member on the node port and is what an enrollment window hands a "
    "joiner. Give every member the same key file -- generate one with `head -c 32 /dev/urandom | base64`, or run "
    "--enroll-from against a member to be handed it";
