...
namespace
{
    ...
    namespace TierRefusal
    {
        ...
        /// The CODE is its own, and this surface needs it more than the daemon does
        ...
        /// therefore where that wrong reading was reached. The counter stays this
        /// tier's own: the daemon answers the same code from its own arm and the two
        /// must not sum, which is `SurfaceRefusal`'s reason for being a row rather
        /// than a lookup on the code.
