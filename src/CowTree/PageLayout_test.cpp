...
TEST_CASE("Leaf page round-trip preserves entries in order", "[pagelayout]")
{
    std::vector<CowTree::LeafEntry> entries {
        { .key = Bytes("apple"), .value = Bytes("red") },
        { .key = Bytes("banana"), .value = Bytes("yellow") },
        { .key = Bytes("cherry"), .value = Bytes("dark-red") },
    };
    std::vector<std::byte> page(4096, std::byte { 0 });
...
TEST_CASE("Leaf page rejects oversized payload", "[pagelayout]")
{
    ...
    std::vector<CowTree::LeafEntry> entries {
        { .key = Bytes("k1"), .value = CowTree::BytesView { big.data(), big.size() } },
        { .key = Bytes("k2"), .value = CowTree::BytesView { big.data(), big.size() } },
        { .key = Bytes("k3"), .value = CowTree::BytesView { big.data(), big.size() } },
    };
    std::vector<std::byte> page(4096, std::byte { 0 });
...
TEST_CASE("Internal page encodes firstChild + entries", "[pagelayout]")
{
    std::vector<CowTree::InternalEntry> entries {
        { .key = Bytes("d"), .child = CowTree::PageId { 2 } },
        { .key = Bytes("m"), .child = CowTree::PageId { 3 } },
    };
    std::vector<std::byte> page(4096, std::byte { 0 });
    REQUIRE(
