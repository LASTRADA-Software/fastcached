...
/// the one-line version and is what this ticket expected to be. Such a string is covered
/// only while its characters are on the heap, and whether they are is a property of the
/// standard library rather than of the program: 15 characters fit inline on libstdc++ 14
/// and 22 on libc++ 22, so the same secret in the same source is heap-allocated on Linux
/// and inline on macOS. A wrapper wiping the inline footprint in its destructor closes less
/// of that than it looks like: a string that GROWS past the threshold copies its characters
/// to the heap and leaves the old ones in the inline buffer, at a moment no destructor and
/// no allocator is told about. There is no hook there, so that residue can only be avoided
/// rather than cleaned.
///
...
class BasicSecureString
{
  public:
    ...
    /// Adopt an allocator instance, for a test that owns the arena behind it.
    /// @param allocator The allocator the characters are taken from.
    explicit BasicSecureString(allocator_type allocator) noexcept:
        _chars { std::move(allocator) }
    {
    }
