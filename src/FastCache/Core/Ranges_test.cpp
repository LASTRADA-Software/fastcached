...
using FastCache::Ranges::Detail::FoldLeftFallback;
using FastCache::Ranges::Detail::IotaFallback;
...
// ---------------------------------------------------------------------------
// `Ranges::Iota` and `Ranges::FoldLeft`.
//
...
TEST_CASE("Iota fallback: writes successive values across a range and says where it stopped", "[core][ranges]")
{
...
TEST_CASE("Iota fallback: stops at a sentinel of another type", "[core][ranges]")
{
...
TEST_CASE("Iota fallback: leaves an empty range alone and hands its value back", "[core][ranges]")
{
...
TEST_CASE("FoldLeft fallback: folds from the left in element order", "[core][ranges]")
{
...
TEST_CASE("FoldLeft fallback: folds an iterator and a sentinel of another type", "[core][ranges]")
{
    auto const numbers = std::vector<int> { 1, 2, 3, 100 };
    CHECK(FoldLeftFallback(std::counted_iterator(numbers.begin(), 3), std::default_sentinel, 0, std::plus {}) == 6);
}

TEST_CASE("FoldLeft fallback: answers in the operation's type when there is nothing to fold", "[core][ranges]")
{
...
TEST_CASE("FoldLeft fallback: reads a single-pass range whose iterator cannot be copied", "[core][ranges]")
{
...
TEST_CASE("Iota and FoldLeft fallbacks: are usable in a constant expression", "[core][ranges]")
{
...
TEST_CASE("Iota fallback: agrees with the standard facility", "[core][ranges]")
{
...
TEST_CASE("FoldLeft fallback: agrees with the standard facility", "[core][ranges]")
{
    auto const halve = [](double accumulated, int element) {
        return (accumulated + element) / 2;
    };
    ...
    CHECK(FoldLeftFallback(numbers, 100, std::minus {}) == std::ranges::fold_left(numbers, 100, std::minus {}));
    ...
    STATIC_REQUIRE(
        std::same_as<decltype(FoldLeftFallback(empty, 7, halve)), decltype(std::ranges::fold_left(empty, 7, halve))>);
}
#endif

TEST_CASE("Ranges seam: is the standard facility exactly where its feature-test macro is defined", "[core][ranges]")
{
    ...
    STATIC_REQUIRE(std::same_as<std::remove_cvref_t<decltype(FastCache::Ranges::Iota)>,
                                std::remove_cvref_t<decltype(std::ranges::iota)>>);
#else
    STATIC_REQUIRE(
        std::same_as<std::remove_cvref_t<decltype(FastCache::Ranges::Iota)>, FastCache::Ranges::Detail::IotaFunction>);
#endif

#if defined(__cpp_lib_ranges_fold) && !defined(FC_RANGES_FORCE_FALLBACK)
    STATIC_REQUIRE(std::same_as<std::remove_cvref_t<decltype(FastCache::Ranges::FoldLeft)>,
                                std::remove_cvref_t<decltype(std::ranges::fold_left)>>);
#else
    STATIC_REQUIRE(std::same_as<std::remove_cvref_t<decltype(FastCache::Ranges::FoldLeft)>,
                                FastCache::Ranges::Detail::FoldLeftFunction>);
#endif
}

TEST_CASE("Ranges seam: answers the calls this tree makes through it", "[core][ranges]")
{
