...
// ---------------------------------------------------------------------------
// C++23 range algorithms that a standard library in this project's build matrix does not
// ship yet: `Ranges::Iota` and `Ranges::FoldLeft`.
//
// ## Why a seam, and not the older spelling
//
// `std::ranges::iota` and `std::ranges::fold_left` say what the code means, and the owner
// chose to keep them rather than retreat to `std::iota` and `std::accumulate` at every call
// site. They are missing from ONE leg: `Package (macOS .pkg)` compiles with AppleClang over
// Apple's own libc++, which answered `error: no member named 'iota' in namespace
// 'std::ranges'` for `DashboardPanel.cpp` on PR #1392, while libstdc++, the MSVC STL and the
// Homebrew LLVM libc++ of `macOS-clang-release` all compiled it. A build stops at its first
// failure, so the `fold_left` in `FrameEndpoint.cpp` was never reached there. That job is not
// a REQUIRED context, so the break was invisible to every gate a branch is merged on.
//
...
// `FoldLeft` fallback even where `std::ranges::fold_left` exists, which costs nothing and
// means both macOS legs compile it in CI and `macOS-clang-release` runs its call sites' tests.
//
// ## Why the fallback is compiled everywhere
//
// `Detail::IotaFallback` and `Detail::FoldLeftFallback` are compiled on every platform,
// selected or not, and `Core/Ranges_test.cpp` calls them directly and asserts they agree
// with the standard facility wherever one exists. A fallback compiled only where it is
// selected is compiled only on the leg nobody runs locally, which is the leg that broke.
//
...
// ## Naming
//
// Project-cased, `Ranges::Iota` rather than `ranges::iota`. `.clang-tidy`'s identifier rules
// then need no `IgnoredRegexp` carve-out, and -- the stronger reason -- a seam spelling that
// differs from the standard one is what lets a case-sensitive scan tell a use of the seam
// from a direct use. The `Ranges` namespace keeps the call site reading as a range algorithm.
//
...
// clang-tidy 22 against this tree's `.clang-tidy`. A copy is a global CONSTANT and takes
// `Iota`. The copy is sound because every library in the matrix defines these objects as
// empty classes with implicit copies -- measured in libstdc++ 14 (`__iota_fn`), libc++ 22
// (`__iota_fn`) and the MSVC STL 14.44 and 14.51 (`_Iota_fn`, `_Fold_left_fn`) -- and a
// library that ever made one uncopyable fails to BUILD here, loudly, rather than misbehaving.
// `Ranges_test.cpp` asserts the TYPE is the standard one wherever it is selected.
//
// ## Enforced
//
// `ctest -R ranges-seam` (`scripts/check-ranges-seam.cmake`) refuses a direct
// `std::ranges::<name>` for every name this header wraps, in first-party C++ outside this
// header and its test. The set of names is DERIVED from the selection blocks below, so a new
// one is enforced without editing the check -- which is also why every block keeps exactly
// its shape: a `#if defined(<macro>) ...` line, then `inline constexpr auto <Name> =
// std::ranges::<name>;` on the line after it. A header the check reads no such pair from
// makes it refuse rather than enforce less.
...
namespace Detail
{
    ...
    struct IotaFunction
    {
        ...
        ///         written next.
        template <std::input_or_output_iterator Out, std::sentinel_for<Out> Sentinel, std::weakly_incrementable Value>
        ...
        constexpr OutValueResult<std::ranges::borrowed_iterator_t<Range>, Value> operator()(Range&& range, Value value) const
        {
            ...
            auto [out, next] = (*this)(std::ranges::begin(target), std::ranges::end(target), std::move(value));
    ...
    concept IndirectlyBinaryLeftFoldable =
        std::copy_constructible<Op> && std::indirectly_readable<It> && std::invocable<Op&, Init, std::iter_reference_t<It>>
        ...
        && IndirectlyBinaryLeftFoldableImpl<Op,
                                            Init,
                                            It,
                                            std::decay_t<std::invoke_result_t<Op&, Init, std::iter_reference_t<It>>>>;
    ...
    struct FoldLeftFunction
    {
        ...
        [[nodiscard]] constexpr auto operator()(Range&& range, Init init, Op op) const
        {
            ...
            return (*this)(std::ranges::begin(source), std::ranges::end(source), std::move(init), std::ref(op));
        }
    };
...
/// not. Call it exactly as the standard facility: `Ranges::Iota(order, std::size_t { 0 });`.
...
/// `Ranges::FoldLeft(counts, std::size_t { 0 }, std::plus {});`.
