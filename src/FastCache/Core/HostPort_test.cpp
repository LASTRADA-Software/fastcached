...
TEST_CASE("An IPv6 literal is split at its bracket, not at its last colon", "[core][hostport]")
{
    ...
    CHECK(bracketed->first == "::1");
    CHECK(bracketed->second == "6674");

    auto const full = SplitHostPort("[fe80::1%eth0]:9");
    REQUIRE(full.has_value());
    CHECK(full->first == "fe80::1%eth0");
...
TEST_CASE("An IPv4 endpoint splits at its only colon", "[core][hostport]")
{
    ...
    CHECK(split->first == "127.0.0.1");
    CHECK(split->second == "6674");
...
{
    CHECK(ParsePort("6674") == std::optional<std::uint16_t> { 6674 });
    CHECK(ParsePort("65535") == std::optional<std::uint16_t> { 65535 });
    ...
    CHECK_FALSE(ParsePort("6674x").has_value());
    CHECK_FALSE(ParsePort(" 6674").has_value());
    CHECK_FALSE(ParsePort("").has_value());
    ...
    CHECK_FALSE(ParsePort("65536").has_value());
    CHECK_FALSE(ParsePort("0").has_value());
    CHECK_FALSE(ParsePort("-1").has_value());
}
...
TEST_CASE("A bare port binds the default host rather than the wildcard", "[core][hostport]")
{
    ...
    CHECK(bare->first == "127.0.0.1");
    CHECK(bare->second == 9100);
    ...
    CHECK(explicitHost->first == "0.0.0.0");
    CHECK(explicitHost->second == 9100);
