...
namespace
{
    ...
    [[nodiscard]] ConfigError MakeError(ConfigErrorCode code,
    ...
    [[nodiscard]] std::expected<LogLevel, ConfigError>
    ParseLogLevel(std::string_view sv, std::filesystem::path const& path, unsigned line)
    {
        ...
        return std::unexpected(MakeError(ConfigErrorCode::OutOfRange,
                                         path,
                                         "log_level",
                                         std::string { "unknown level: " } + std::string { sv },
                                         line));
    }
...
std::expected<Config, ConfigError> ReadYamlConfig(std::filesystem::path const& path)
{
    ...
    catch (YAML::ParserException const& e)
    {
        return std::unexpected(MakeError(
            ConfigErrorCode::ParseError, path, {}, e.msg, static_cast<unsigned>(e.mark.line + 1)));
    }
    ...
    if (root.IsMap())
    {
        for (auto const& kv : root)
        {
            ...
            else if (key == "port")
            {
                ...
                if (raw <= 0 || raw > 65535)
                    return std::unexpected(MakeError(
                        ConfigErrorCode::OutOfRange, path, "port", "must be in 1..65535", line));
            ...
            else if (key == "max_memory")
            {
                ...
                if (raw < 0)
                    return std::unexpected(MakeError(
                        ConfigErrorCode::OutOfRange, path, "max_memory", "must be >= 0", line));
            ...
            {
                return std::unexpected(MakeError(
                    ConfigErrorCode::UnknownKey, path, key, "unrecognised key", line));
            }
        }
    }
