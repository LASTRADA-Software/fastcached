...
/// object whose lifetime a call site could get wrong. @p report is MOVED into that
/// subscription and is owned by it too -- what a call site still owns is whatever the
/// callable REFERS to, and that is what has to outlive @p reloader, because the
/// subscription calls it from `Reload()`. Stating the requirement against @p report
/// itself would be unfollowable as written: the argument is ordinarily a temporary
/// lambda that dies at the end of the call. `DaemonBody` is the shape to copy -- it
/// hands over `[&logger]` and declares `logger` BEFORE the reloader, so the referent
/// is destroyed after it.
///
/// **It takes the provenance BIT, never the whole `CliResult`.** The subscriber lives
/// as long as the daemon and `ConfigReloaderOf::Reload()` deep-copies every subscriber
/// on each SIGHUP, so whatever this captures is duplicated per reload: measured, 7
/// allocations and 828 bytes for a captured `CliResult` -- which carries a whole
/// `Config` -- against 1 allocation and 56 bytes for the one bool the rule reads.
/// Copied rather than referenced either way, because argv does not change under a
/// running process and a reload must ask exactly the question the start asked.
///
