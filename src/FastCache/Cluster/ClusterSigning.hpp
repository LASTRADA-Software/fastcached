...
enum class SigningDomain : std::uint8_t
{
    ...
    /// the Raft peer handshake. There a verdict is SIGNED because both ends must agree about
    /// the connection and an unsigned refusal is a confident wrong signal. Here the proof is
    /// an OPTIONAL upgrade: a caller that presents none is admitted or refused by address
    /// exactly as before, so the only thing a refusal has to reach is this server's own
    /// counters.
    ///
    /// The Raft peer wire had three rows here until #178, when its handshake moved to each
    /// node's OWN key and this key stopped signing anything about consensus. Their labels are
