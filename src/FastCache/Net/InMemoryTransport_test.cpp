...
TEST_CASE("InMemorySocketPair shuttles bytes both directions", "[net][inmemory]")
{
    ...
    REQUIRE(FastCache::SyncRun(WriteString(*pair.client, "hello")));

    auto received = FastCache::SyncRun(ReadAllAvailable(*pair.server, 5));
    REQUIRE(received == "hello");

    REQUIRE(FastCache::SyncRun(WriteString(*pair.server, "world!")));
    received = FastCache::SyncRun(ReadAllAvailable(*pair.client, 6));
...
{
    auto pair = FastCache::InMemorySocketPair::Create();
    REQUIRE(FastCache::SyncRun(WriteString(*pair.client, "ab")));
    pair.client->Close();

    // Drain the two bytes, then expect EOF on the next read.
    auto received = FastCache::SyncRun(ReadAllAvailable(*pair.server, 4));
    REQUIRE(received == "ab");
    ...
    auto const result =
        FastCache::SyncRun([](FastCache::ISocket& socket, std::span<std::byte> buffer) -> FastCache::Task<bool> {
            ...
            REQUIRE(r.has_value());
            co_return *r == 0;
        }(*pair.server, std::span<std::byte> { tail, 1 }));
    REQUIRE(result);
}
...
TEST_CASE("InMemoryPipe respects the backpressure cap", "[net][inmemory]")
{
    ...
    bool clientWroteOk = FastCache::SyncRun(WriteString(*pair.client, "1234"));
