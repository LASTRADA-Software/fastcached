...
namespace
{
    ...
    {
        co_return co_await Detail::DialReadiness<KqueueDialTraits>(
            static_cast<KqueueReactor*>(state), endpoint, deadline, keepAlive);
    }
...
Task<SocketResult> KqueueConnector::Connect(std::string host, std::uint16_t port, DialOptions options)
{
