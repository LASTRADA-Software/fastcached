...
[[nodiscard]] inline DWORD WsaErrorOf(SOCKET socket, IocpCompletion& completion, IocpStatus status) noexcept
{
    ...
    // **Winsock has just said the operation SUCCEEDED, so this answers 0.**
    //
    // It is rarer than it is impossible, and the reason is that `Failed()` above is
    // `_status != 0` while not every non-zero NTSTATUS is a failure: the severity lives
    // in the top two bits, and the warning-severity codes -- `STATUS_BUFFER_OVERFLOW`,
    // 0x80000005, and its neighbours -- are successes carrying a note. On one of those
    // `Failed()` says yes, `WSAGetOverlappedResult` says the operation completed, and
    // answering `ERROR_OPERATION_ABORTED` here would turn a completed transfer into a
    // `Cancelled` read and throw away the bytes it moved.
    //
    // That is the same defect as the one `IocpSocket::CancelRead` guards against by
    // asking what the operation did rather than assuming it lost -- discarding a real
    // result because a cancellation was expected. Answering 0 lets the byte count the
    // completion carries stand, which is what actually happened.
    //
    // Not the raw NTSTATUS either, which is what the single-site version this replaces
    // did: that puts the untranslated number back into the taxonomy through the one path
    // nobody tests.
    return 0;
}
