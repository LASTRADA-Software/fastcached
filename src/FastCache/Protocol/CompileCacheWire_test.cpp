          ...
          "[wire][consensus][nodestatus]")
{
    ...
    auto const parts = WireFields::SplitAll(emitted);
    REQUIRE(parts.has_value());
    // Sixteen: thirteen that predate #1328, the endpoint it added, #1471's applied-tombstone count,
    // and #1364's condition list. Pinned, since every cut below is counted from it and a record that
    // grew would move what "older" means -- which is how this case caught both #1471's append and
    // #1364's rather than letting either shift the cuts silently.
    ...
    SECTION("fifteen fields, as a build after #1471 and before #1364 emits: the conditions are absent")
    {
        // The cut #1364 has to survive: a peer that knows the tombstone count and has never heard
        // of conditions. Its list must come back ABSENT -- a node too old to say, which every
    ...
    SECTION("sixteen fields, this build: every fact engaged")
    {
