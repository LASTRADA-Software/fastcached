...
class ShardedStorage final: public IStorage
{
  public:
    ...
    /// Callers wiring this up should typically not call shard backends'
    /// Resize directly; the budget passed here is the total budget
    /// across all shards.
    /// @param newTotalBytes New total byte budget (sum across all shards).
    /// @note Shards must expose a Resize method; the decorator dispatches
    ///       via dynamic_cast<>. Unknown shard types are silently
    ///       ignored.
    void ResizeTotal(std::size_t newTotalBytes);
