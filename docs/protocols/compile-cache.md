...
| `0x1c` | lease-foreign-cluster | An authentic lease, issued by a scheduler belonging to a **different fleet**. Authentic and unauthorised at once, which is why it is neither `lease-unauthorized` nor a mismatch: the MAC verified, so both fleets hold the same pre-shared key — and that is the finding. Two clusters provisioned from one `--cluster-key-file`, the ordinary result of copying a working configuration to a second site, otherwise honour each other's grants perfectly. The message names both fleet ids and the shared key file, because an operator who has just cloned a configuration will otherwise look at the network. |
| `0x1d` | lease-stale-epoch | An authentic lease, issued under a scheduler epoch older than one this worker has already served. A grant minted before a leadership change, presented after it. Distinct from `lease-expired`: the token may be well inside its expiry, and what makes it stale is that the fleet moved on rather than that time passed. |
